...
Most requests use `x-api-key`; session-only routes are marked per endpoint.
Responses are JSON by default, except export and download endpoints that return
their requested file format. HTTPS only.
...
Require explicit user confirmation for the exact key before revoking it.
...
Run a bulk data extraction job. See `./extractions.md` for all 23 tool types.
...
First download is metered. Subsequent requests for the same tweet return cached URLs at no cost (`cacheHit: true`). All downloads are saved to the gallery at `https://xquik.com/g/{share-token}`.
...
Get trending topics and news from curated public signals. Free.

**Query parameters:**

| Param | Type | Description |
|-------|------|-------------|
| `source` | string | Optional source filter. See the live docs for accepted values. |
