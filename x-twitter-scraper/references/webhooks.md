...
async function processOnce(payload, res, store) {
  const payloadHash = createHash("sha256").update(payload).digest("hex");
  if (await store.has(payloadHash)) {
    return res.status(200).send("Already processed");
  }

  await processVerifiedEvent(JSON.parse(payload));
  await store.add(payloadHash);
  return res.status(200).send("OK");
}
```

Use a durable `store` backed by Redis or a database. Do not use process memory,
because restarts would lose deduplication state.
