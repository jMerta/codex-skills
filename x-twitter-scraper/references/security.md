...
- search queries: bounded length, URL encoded before transport, and no control characters
- webhook URLs: valid HTTPS URLs; optionally check DNS or reachability before creating persistent delivery
- date ranges: ISO 8601 or documented endpoint format, sensible bounds, and `start <= end`
- event types: one of `tweet.new`, `tweet.quote`, `tweet.reply`, `tweet.retweet`, or `webhook.test`
