...
| Trending news from curated signals | `GET /api/v1/radar` (via `xquik` tool) |
...
| **Get trending news** | `GET /radar` (free, via `xquik` tool) -> `POST /compose` with trending topic |
...
- **Free**: account info, compose (all steps), styles (cached lookup/save/delete/compare), drafts, radar (via `xquik` tool), support tickets, credits balance check, webhook management
