...
>
> In shells, set the key outside the config file: `export XQUIK_API_KEY="..."` on macOS/Linux or `setx XQUIK_API_KEY "..."` on Windows. Prefer your agent or IDE secret manager when it supports one.
...
> Set the `XQUIK_API_KEY` environment variable before launching Claude Desktop. Do not replace `${XQUIK_API_KEY}` with a literal key in a committed config file.
...
> **Security:** The `x-api-key` value below is a placeholder. Replace it only in a local, ignored config or use environment variable or dashboard-based secret injection when your platform supports it. Never commit `.mcp.json` with a real key.
...
> Store this in your local user config only. Prefer secret-manager or environment-variable support when available, and never commit a real `x-api-key`.
...
> If you use a project config, keep it ignored and do not commit a real key.
...
> Keep this in user-level config or an ignored local file. Do not commit a real key.
...
- "What's trending in developer news today?"
