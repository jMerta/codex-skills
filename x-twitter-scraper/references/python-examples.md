...
def xquik_fetch(path, method="GET", json_body=None, max_retries=3):
    ...
    for attempt in range(max_retries + 1):
        ...
        if not retryable or attempt == max_retries:
            ...
            raise Exception(f"Xquik API {response.status_code}: {message}")
...
```python
import hmac, hashlib, json, os, sqlite3
from flask import Flask, request

app = Flask(__name__)
# Per-webhook secret from POST /webhooks response, not a Xquik account credential.
# Store secrets per webhook ID when one app handles multiple endpoints.
WEBHOOK_SECRET = os.environ["XQUIK_WEBHOOK_SECRET"]
conn = sqlite3.connect("xquik_webhook_events.sqlite3", check_same_thread=False)
conn.execute("CREATE TABLE IF NOT EXISTS processed_events (payload_hash TEXT PRIMARY KEY)")
conn.commit()
...
def is_processed(payload_hash: str) -> bool:
    row = conn.execute(
        "SELECT 1 FROM processed_events WHERE payload_hash = ?",
        (payload_hash,),
    ).fetchone()
    return row is not None

def mark_processed(payload_hash: str) -> None:
    conn.execute(
        "INSERT OR IGNORE INTO processed_events (payload_hash) VALUES (?)",
        (payload_hash,),
    )
    conn.commit()
