...
If `bun` is not available, tell the user and ask whether to install it or proceed with manual steps. If `bash` is not available (common on Windows), ask whether to use WSL/Git Bash or run the commands manually (`bun install`, then `bun run start-server`).
...
> **CRITICAL: Always run scripts from the dev-browser skill directory (e.g., `~/.codex/skills/dev-browser/`)**
>
> Scripts must be executed from the dev-browser skill directory. The `@/` import alias (e.g., `@/client.js`) is configured in this directory's `tsconfig.json` and `package.json`. Running from any other directory will fail with:
>
> ```
...
```bash
cd ~/.codex/skills/dev-browser && bun x tsx <<'EOF'
...
## Deliverable
Provide:
- The server status (started/stopped) and any flags used (e.g., `--headless`).
- The scripts executed (inline or saved under `tmp/`) and their outputs/logs.
- Any screenshots or artifacts created (paths).
- Next-step recommendation if the workflow needs follow-up scripts.
import { connect } from "@/client.js";
